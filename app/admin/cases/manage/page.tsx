...
export default function ManageCasesPage() {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setStatus(`수정 실패: ${data.error || '알 수 없는 오류'}`);
        return;
      }
      setStatus('수정 완료!');
      setEditingId(null);
  ...
  const deleteCase = async (caseId: number) => {
    ...
    setStatus('삭제 중...');
    try {
      const res = await fetch(`/api/cases/${caseId}?secret=${encodeURIComponent(secret)}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (!res.ok) {
        setStatus(`삭제 실패: ${data.error || '알 수 없는 오류'}`);
        return;
      }
  ...
  const inputStyle: React.CSSProperties = {
    width: '100%', padding: '8px 10px', marginBottom: 10, border: '1px solid #ddd', borderRadius: 6, fontSize: 13,
  };
    ...
    return (
      <div style={{ maxWidth: 400, margin: '100px auto', padding: 20 }}>
        ...
        <button
          ...
          style={{ padding: '10px 20px', background: '#1a1a2e', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer' }}
        >
          입장
        </button>
      </div>
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 20px' }}>
      ...
      {cases.map((c) => (
        <div key={c.caseId} style={{ border: '1px solid #eee', borderRadius: 8, padding: 16, marginBottom: 12 }}>
          {editingId === c.caseId ? (
            <div>
              <label style={{ fontSize: 12, fontWeight: 600 }}>사건 종류</label>
              <input
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                <button onClick={saveEdit} style={{ padding: '8px 16px', background: '#1a1a2e', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer' }}>
                  저장
                </button>
                <button onClick={cancelEdit} style={{ padding: '8px 16px', background: '#eee', border: 'none', borderRadius: 6, cursor: 'pointer' }}>
                  취소
                </button>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              {c.image && (
                <img src={c.image} alt="" style={{ width: 100, height: 75, objectFit: 'cover', borderRadius: 6, flexShrink: 0 }} />
              )}
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12, color: '#888' }}>#{c.caseId} · {c.createdAt?.slice(0, 10)}</div>
                <div style={{ fontWeight: 600, marginBottom: 4 }}>{c.caseType}</div>
                <div style={{ fontSize: 13, color: '#555' }}>{c.summary?.slice(0, 80)}...</div>
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  <button onClick={() => startEdit(c)} style={{ padding: '6px 14px', background: '#eee', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 13 }}>
                    수정
                  </button>
                  <button onClick={() => deleteCase(c.caseId)} style={{ padding: '6px 14px', background: '#fee', color: '#c33', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 13 }}>
                    삭제
                  </button>
                </div>
              </div>
