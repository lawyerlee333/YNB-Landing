...
async function generateArticle(recentTopics: string[]) {
  ...
  const prompt = `너는 법무법인 와이앤비의 법률 블로그 필진이야.
...
- 분량: 800~1200자
- 제목 형식은 반드시 "법무법인 와이앤비 : 주제" 형태로 고정 (콜론 + 공백 구분자, Y&B·전주Y&B·| 등 사용 금지)
- 본문과 제목에 **, ##, -, * 같은 마크다운 기호 절대 사용 금지
- AI가 쓴 티가 나는 문체(예: "~해 드립니다", "~하겠습니다" 반복, 과도한 나열) 지양
- 자연스러운 블로그 글체로 작성
